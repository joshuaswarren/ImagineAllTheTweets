...
function getUrlContents($url) {
	...
	$contents = $res;
	$m->set($url, $contents);
	echo "Added $url to cache\n";
	return $res;
}

function checkContentsForOgImage($contents) {
	$pos = strpos($contents, 'og:image');
	if($pos !== false) {
		$httpPos = strpos($contents, 'http', $pos);
		$nextQuote = strpos($contents, '"', $httpPos);
		$length = $nextQuote - $httpPos;
		$imageLink = substr($contents, $httpPos, $length);
		// echo "Found og:image $imageLink\n";
		return $imageLink;
	}
	return null;
}

function checkContentsForPicTwitterImage($contents) {
	$pos = strpos($contents, 'img src="https://pbs.twimg.com/media/');
	if($pos !== false) {
...
function isYfrogOrInstagram($url) {
	if(strpos($url, 'yfrog') !== false)  {
		return true;
	}
	if(strpos($url, 'instagram') !== false) {
		// echo "Looks to be Instagram\n";
		return true;
	}
	return null;	
}

function checkContentsForImages($contents, $url) {
	// echo "Checking contents for images\n";
	$picTwitterPhoto = checkContentsForPicTwitterImage($contents);
	if(!is_null($picTwitterPhoto)) {
		return $picTwitterPhoto;
	}
	...
	$ogImage = checkContentsForOgImage($contents);
	if(!is_null($ogImage)) {
		$frog = isYfrogOrInstagram($url); 
		var_dump($frog);
		echo "\n";
	}
	if(!is_null($ogImage) && !is_null(isYfrogOrInstagram($ogImage))) {
		return $ogImage;
	}	
	return null;	
}
...
function checkUrlsForImages($mi) {
	...
	while($row = $res->fetch_assoc()) {
		...
		$image = checkContentsForImages($contents, $url);
		if(!is_null($image)) {
...
function insertImage($id, $url, $mi) {
	...
	$query = 'INSERT INTO tweet_urls (tweet_id, url) VALUES (' . $id . ', "' . $url . '")';
	$res = $mi->query($query);
		if($res === false) {
		// echo "Error in $query " . $mi->error . "\n";
	}
