...
(function ($, window) {
	...
	MapBubble.prototype.onAdd = function () {
		...
		if (this.shadowImg) {
			this.shadow = $('<div class="tweetmap-shadow"></div>');
		...
		this.container.append('<div class="tweetmap-loading"></div>');
		this.contentContainer = $('<div class="tweetmap-panel"></div>');
		this.container.append(this.contentContainer);
		this.container.append('<div class="tweetmap-pointer">▲</div><div class="tweetmap-close"><a href="#">x</a></div>');
		...
		$('.tweetmap-close > a', this.container).click(close);
	};
	...
	var createSearchURL = function (options) {
		...
		return 'tweetmap/twitter/proxy.php?' + items.join('&') + '&callback=?';
	},

		getFeed = function (url, options, map, thisMap) {

			$(thisMap).append('<div class="tweetmap-mapLoading">Loading Tweets...</div>');
			...
			$.getJSON(url, function (data) {
				...
				$('.tweetmap-mapLoading', thisMap).remove();
